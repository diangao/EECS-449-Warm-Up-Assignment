...
class greet_user(_Jac.Walker):
    ...
    def return_message(self, _jac_here_: _Jac.RootType) -> None:
        _Jac.report({'response': 'Hello, ' + self.name + '! How can I assist you today?'})

@_Jac.make_walker(on_entry=[_Jac.DSFunc('return_message', _Jac.RootType)], on_exit=[])
@__jac_dataclass__(eq=False)
class get_time(_Jac.Walker):

    def return_message(self, _jac_here_: _Jac.RootType) -> None:
        if _jac_typ.TYPE_CHECKING:
            import time
        else:
            time, = __jac_import__(target='time', base_path=__file__, lng='py', absorb=False, mdl_alias=None, items={})
        current_time = time.strftime('%H:%M:%S')
        _Jac.report({'response': 'The current time is ' + current_time})